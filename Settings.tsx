...
import { Save, Plus, Trash2, Shield, Users, Church, Coins, Settings as SettingsIcon, Info } from 'lucide-react';
...
export const Settings: React.FC<SettingsProps> = ({
  ...
  // ✅ campo de senha só para criação — nunca exibido, nunca armazenado no estado global
  ...
  const handleAddAdm = () => {
    if (newAdminName.trim() && newAdminEmail.trim() && newAdminPass.trim()) {
      onAddAdmin({ name: newAdminName.trim(), email: newAdminEmail.trim(), password: newAdminPass });
  ...
  return (
    <div className="space-y-8 pb-20 animate-fade-in relative z-10">
      ...
      <div className="bg-zinc-900/80 backdrop-blur-xl p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
        <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-green-500/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
      ...
      <div className="bg-zinc-900/80 backdrop-blur-xl p-6 rounded-2xl border border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Pontos por Categoria</p>
          ...
          <div className="space-y-6">
            <div className="bg-yellow-500/5 p-5 rounded-2xl border border-yellow-500/10 text-center">
              ...
              <p className="text-xs text-zinc-500 mt-4">100 pontos = R$ {(100 * localPointsValue).toFixed(2)} de desconto.</p>
            </div>
            <button onClick={handleSavePoints} className="w-full bg-yellow-600 text-white py-4 rounded-xl font-bold hover:bg-yellow-500 transition-all shadow-lg flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-zinc-900/80 backdrop-blur-xl p-6 rounded-2xl border border-white/10">
          ...
          <div className="flex gap-2 mb-4">
            <input value={newVol} onChange={e => setNewVol(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddVol()} placeholder="Nome do voluntário..." className="flex-1 bg-black/50 border border-white/10 rounded-xl px-4 py-2 text-sm text-white outline-none focus:border-blue-500" />
        ...
        <div className="bg-zinc-900/80 backdrop-blur-xl p-6 rounded-2xl border border-white/10">
          ...
          <div className="flex gap-2 mb-4">
            <input value={newService} onChange={e => setNewService(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddServ()} placeholder="Nome do culto..." className="flex-1 bg-black/50 border border-white/10 rounded-xl px-4 py-2 text-sm text-white outline-none focus:border-purple-500" />
      ...
      {/* ADMINS */}
      <div className="bg-zinc-900/80 backdrop-blur-xl p-6 rounded-2xl border border-white/10">
        <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-2 border-b border-white/5 pb-4">
        ...
        <div className="flex items-start gap-3 bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 mb-6">
          ...
          <p className="text-xs text-blue-300">
            A autenticação agora é feita via <strong>Supabase Auth</strong>. Para adicionar um admin,
            crie o usuário no <strong>Supabase Dashboard → Authentication → Users</strong> e depois
            adicione o e-mail aqui para vincular à unidade.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 items-end bg-black/20 p-4 rounded-xl border border-white/5">
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-zinc-500 uppercase">Senha Supabase Auth</label>
            <input type="password" value={newAdminPass} onChange={e => setNewAdminPass(e.target.value)} className="w-full bg-black border border-zinc-800 rounded-lg px-3 py-2 text-sm text-white focus:border-red-500 outline-none" placeholder="******" />
          </div>
            ...
            <div key={admin.id} className="flex justify-between items-center bg-black/40 p-4 rounded-xl border border-white/5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500">{admin.email} {admin.role ? `• ${admin.role}` : ''}</p>
                </div>
              </div>
