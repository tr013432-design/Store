...
import { supabase } from './supabaseClient'; // Conexão com o Banco de Dados
...
import { LayoutDashboard, Package, Menu, CheckCircle, ShoppingBag, Settings as SettingsIcon, Lock, Mail, Key, LogOut, Truck, Star, Users, TrendingDown, Store, ChevronRight, MapPin, Plus, X, Trash2, Download } from 'lucide-react';
...
const StoreSystem: React.FC<{ unitId: string, unitName: string, onLogoutUnit: () => void }> = ({ unitId, unitName, onLogoutUnit }) => {
  ...
  // Configurações Locais (Pode manter local por enquanto)
  const [pointsConfig, setPointsConfig] = useState<Record<string, number>>({ 'Livros e Bíblias': 15, 'Vestuário': 30, 'Papelaria': 5, 'Acessórios': 5, 'Outros': 1 });
  ...
  const fetchData = async () => {
    setLoading(true);
    // Busca tudo do Supabase filtrando pelo unit_id (Igreja atual)
  ...
  useEffect(() => {
      const txReports = reports.filter(r => r.status === 'VALIDADO').map(r => ({
      ...
      const txOrders = orders.filter(o => o.status === 'ENTREGUE').map(o => ({
  ...
  const handleAddProduct = async (prod: Product) => {
      const { id, ...newProd } = prod; // Remove ID temporário se tiver
  ...
  const handleUpdateProduct = async (prod: Product) => {
      ...
      if (!error) setProducts(products.map(p => p.id === prod.id ? prod : p));
  };
  ...
  const handleValidateReport = async (id: string, admin: string) => {
    ...
    // 2. Baixa Estoque (Lógica simplificada no Front por enquanto)
  ...
  const handleAddExpense = async (newExp: Expense) => {
      const { data } = await supabase.from('expenses').insert([{ ...newExp, unit_id: unitId }]).select();
  ...
  const handleSaveCustomer = async (c: Customer) => {
      const existing = customers.find(cust => cust.phone === c.phone);
      if (existing) {
          const { error } = await supabase.from('customers').update(c).eq('id', existing.id);
          if (!error) setCustomers(prev => prev.map(x => x.id === existing.id ? { ...x, ...c } : x));
      } else {
          // Remove ID se for novo para o banco gerar uuid
          const { id, ...newCust } = c;
  ...
  // --- PLACEHOLDERS (Funções que ainda não afetam banco crítico) ---
  const handleValidateOrder = (id: string, admin: string) => { setOrders(p => p.map(o => o.id === id ? {...o, status: 'ENTREGUE', validatedBy: admin} : o)); alert("Encomenda Entregue!"); };
  ...
  const handleMarkItemDelivered = (oid: string, iid: string) => {};
  const handleManualAddPoints = () => {}; const handleRedeemReward = () => {};
  const addVolunteer = (n: string) => setAvailableVolunteers(p => [...p, n]); const removeVolunteer = (n: string) => setAvailableVolunteers(p => p.filter(v => v !== n));
  const addService = (s: string) => setAvailableServices(p => [...p, s]); const removeService = (s: string) => setAvailableServices(p => p.filter(x => x !== s));
  const addAdmin = (a: any) => setAdmins(p => [...p, { ...a, id: Date.now().toString() }]); const removeAdmin = (id: string) => setAdmins(p => p.filter(a => a.id !== id));
    ...
    <div className="min-h-screen bg-zinc-950 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-zinc-900 via-zinc-950 to-black text-zinc-100 font-sans selection:bg-green-500/30 selection:text-green-200">
      <aside className="hidden lg:flex flex-col w-72 bg-black/40 border-r border-white/5 p-4 fixed h-full z-20 backdrop-blur-xl">
        ...
        <div className="p-4 mt-4 border-t border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-gradient-to-br from-green-500 to-green-800 flex items-center justify-center text-xs font-bold shadow-lg shadow-green-500/20">A</div><div><p className="text-xs font-bold text-white">Admin</p><p className="text-[10px] text-zinc-500">Logado</p></div></div>
      ...
      {mobileMenuOpen && (<div className="lg:hidden fixed inset-0 z-40 bg-black/90 backdrop-blur-xl" onClick={() => setMobileMenuOpen(false)}><div className="bg-zinc-950 w-3/4 h-full p-6 border-r border-white/10 shadow-2xl" onClick={e => e.stopPropagation()}><SidebarContent /></div></div>)}
      ...
      <main className="flex-1 lg:ml-72 p-4 lg:p-10 pt-24 lg:pt-10 transition-all min-h-screen relative">
        <div className="max-w-7xl mx-auto animate-fade-in relative z-10">
                ...
                !isDashboardUnlocked ? (
                    <div className="flex flex-col items-center justify-center h-[80vh]">
                        <div className="bg-zinc-900/80 p-10 rounded-3xl border border-white/10 max-w-sm w-full text-center relative overflow-hidden group">
                            ...
                            <input value={loginEmail} onChange={e => setLoginEmail(e.target.value)} className="w-full bg-black border border-zinc-800 rounded-xl px-4 py-3 mb-3 text-white focus:border-green-500 outline-none transition-all" placeholder="Email" />
                            <input type="password" value={loginPass} onChange={e => setLoginPass(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleDashboardLogin()} className="w-full bg-black border border-zinc-800 rounded-xl px-4 py-3 mb-6 text-white focus:border-green-500 outline-none transition-all" placeholder="Senha" />
                            <button onClick={handleDashboardLogin} disabled={isLoadingLogin} className="w-full bg-green-600 text-white py-3 rounded-xl font-bold hover:bg-green-500 transition-all">{isLoadingLogin ? 'Entrando...' : 'Acessar'}</button>
                        </div>
                    </div>
                ) : (
                    <div className="relative">
                        <div className="absolute top-0 right-0 z-10"><button onClick={() => setIsDashboardUnlocked(false)} className="bg-black/40 px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 border border-white/5 hover:bg-white/5 transition-all"><LogOut size={14} /> Bloquear</button></div>
          ...
          {!loading && currentView === View.VOLUNTEER_REPORT && <VolunteerSales products={products} onSubmitReport={handleReportSubmit} availableVolunteers={availableVolunteers} availableServices={availableServices} customers={customers} pointsValue={pointsValue} />}
          {!loading && currentView === View.ORDERS && <Orders products={products} onSubmitOrders={handleOrderSubmit} availableVolunteers={availableVolunteers} availableServices={availableServices} />}
          {!loading && currentView === View.VALIDATION && <ReportValidation reports={reports} orders={orders} admins={admins} onValidateReport={handleValidateReport} onValidateOrder={handleValidateOrder} onUnvalidateReport={handleUnvalidateReport} onUnvalidateOrder={handleUnvalidateOrder} onToggleReportItem={handleToggleReportItem} onToggleOrderItem={handleToggleOrderItem} />}
          ...
          {!loading && currentView === View.LOYALTY && <Loyalty customers={customers} pointsConfig={pointsConfig} onUpdatePointsConfig={setPointsConfig} onManualAddPoints={handleManualAddPoints} onRedeemReward={handleRedeemReward} />}
          {!loading && currentView === View.INVENTORY && <Inventory products={products} onUpdateProduct={handleUpdateProduct} onAddProduct={handleAddProduct} onDeleteProduct={handleDeleteProduct} />}
          ...
          {!loading && currentView === View.SETTINGS && <Settings volunteers={availableVolunteers} services={availableServices} admins={admins} pointsConfig={pointsConfig} pointsValue={pointsValue} onUpdatePointsConfig={setPointsConfig} onUpdatePointsValue={setPointsValue} onAddVolunteer={addVolunteer} onRemoveVolunteer={removeVolunteer} onAddService={addService} onRemoveService={removeService} onAddAdmin={addAdmin} onRemoveAdmin={removeAdmin} />}
        </div>
      </main>
    </div>
  );
...
const App: React.FC = () => {
    // Busca Regionais direto do Banco
    ...
    const handleDeleteUnit = async (e: React.MouseEvent, id: string) => {
        ...
        if (senha === '123') { // Senha de segurança simples para deletar
             ...
             <div className="relative z-10 w-full max-w-5xl text-center">
                 ...
                 {loadingUnits ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                        {units.map((unit) => (
                            <button key={unit.id} onClick={() => { setLoginModalUnit(unit); setLoginPassword(''); }} className="group relative bg-zinc-900/50 hover:bg-zinc-800 backdrop-blur-xl border border-zinc-800 hover:border-green-500/50 rounded-2xl p-6 transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-green-900/20 flex flex-col items-center gap-4 text-left">
                                <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity"><div onClick={(e) => handleDeleteUnit(e, unit.id)} className="p-2 hover:bg-red-500/20 text-zinc-600 hover:text-red-500 rounded-full transition-colors"><Trash2 size={14}/></div></div>
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl font-bold bg-zinc-950 border border-zinc-800 group-hover:border-green-500 group-hover:text-green-400 transition-colors shadow-lg`}><MapPin size={20} className="text-zinc-500 group-hover:text-green-500 transition-colors" /></div>
                                <div className="relative z-10"><h3 className="text-white font-bold text-lg group-hover:text-green-400 transition-colors">{unit.name}</h3><div className="flex items-center justify-center gap-1 mt-1"><Lock size={10} className="text-zinc-600 group-hover:text-green-600"/><p className="text-zinc-600 text-xs font-medium uppercase tracking-wider group-hover:text-zinc-400">Protegido</p></div></div>
                            </button>
                        ))}
                        <button onClick={() => setAddModalOpen(true)} className="group relative bg-zinc-950/30 hover:bg-zinc-900 border border-dashed border-zinc-800 hover:border-zinc-600 rounded-2xl p-6 transition-all flex flex-col items-center justify-center gap-4 text-center min-h-[160px]">
                            <div className="w-12 h-12 rounded-full flex items-center justify-center bg-zinc-900 group-hover:bg-zinc-800 transition-colors"><Plus size={24} className="text-zinc-500 group-hover:text-white" /></div>
             ...
             {loginModalUnit && (
                 <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
                     <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center relative overflow-hidden animate-fade-in">
                        ...
                        <input autoFocus type="password" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleConfirmLogin()} className="w-full bg-black border border-zinc-700 rounded-xl px-4 py-3 text-white text-center tracking-widest outline-none focus:border-green-500 mb-4" placeholder="••••" />
                        <div className="flex gap-3"><button onClick={() => setLoginModalUnit(null)} className="flex-1 py-3 text-zinc-500 hover:text-white font-bold transition-colors">Cancelar</button><button onClick={handleConfirmLogin} className="flex-1 bg-green-600 text-white py-3 rounded-xl font-bold hover:bg-green-500 transition-colors">Acessar</button></div>
                     </div>
                 </div>
             )}
                 ...
                 <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
                     <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center relative overflow-hidden animate-fade-in">
                        ...
                        <div className="space-y-4 text-left">
                            <div><label className="text-xs font-bold text-zinc-500 uppercase ml-1">Nome da Regional</label><input value={newUnitName} onChange={e => setNewUnitName(e.target.value)} className="w-full bg-black border border-zinc-700 rounded-xl px-4 py-3 text-white outline-none focus:border-green-500" placeholder="Ex: Bangu" /></div>
                            <div><label className="text-xs font-bold text-zinc-500 uppercase ml-1">Senha de Acesso</label><input type="password" value={newUnitPass} onChange={e => setNewUnitPass(e.target.value)} className="w-full bg-black border border-zinc-700 rounded-xl px-4 py-3 text-white outline-none focus:border-green-500" placeholder="Crie uma senha segura" /></div>
