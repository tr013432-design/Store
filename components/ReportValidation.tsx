...
import { CheckCircle, ShoppingBag, Phone, User, Users, FileText, Filter, X, ChevronDown, ChevronUp, RotateCcw, CheckSquare, Square } from 'lucide-react';
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 bg-white p-2 rounded-xl shadow-sm border border-slate-200">
            ...
            <select value={volunteerFilter} onChange={e => setVolunteerFilter(e.target.value)} className="bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-lg p-2 outline-none">
                {uniqueVolunteers.map(v => <option key={v} value={v}>{v === 'Todos' ? 'Todos Voluntários' : v}</option>)}
            </select>
            <select value={serviceFilter} onChange={e => setServiceFilter(e.target.value)} className="bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-lg p-2 outline-none">
        ...
        {pendingReports.length === 0 ? <p className="text-sm text-slate-400 italic">Nenhum relatório pendente.</p> : 
            pendingReports.map(report => (
                <ReportCard 
                    ...
                    onAction={() => onValidateReport(report.id)} 
                    onToggleItem={onToggleReportItem} // Passando função de toggle
                />
            ))
        }
      </div>
        ...
        <div className="mt-6">
            ...
            {pendingOrders.map(order => (
                <OrderCard 
                    ...
                    onAction={() => onValidateOrder(order.id)}
                    onToggleItem={onToggleOrderItem} // Passando função de toggle
                />
            ))}
        </div>
      )}
        ...
        <div className={`mb-3 bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${isExpanded ? (isVal ? 'shadow-md border-emerald-200' : 'shadow-lg border-amber-400 ring-1 ring-amber-400') : 'shadow-sm border-slate-200'}`}>
            ...
            {isExpanded && (
                <div className="border-t border-slate-100 bg-white p-4 animate-fade-in">
                        ...
                        <table className="w-full text-xs text-left">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {report.items.map((i: any, idx: number) => (
                                    <tr 
                                        key={idx} 
                                        onClick={() => !isVal && onToggleItem && onToggleItem(report.id, idx)}
                                        className={`transition-colors ${!isVal ? 'cursor-pointer hover:bg-white' : ''} ${i.checked ? 'bg-emerald-50/50' : ''}`}
                                    >
                                        {!isVal && (
                                            <td className="p-2">
                                                {i.checked 
                                        ...
                                        <td className="p-2 font-medium">{i.quantity}</td>
                                        <td className={`p-2 ${i.checked ? 'text-emerald-700 font-medium' : 'text-slate-600'}`}>
                                            {i.productName} <span className="text-[10px] opacity-70">({i.paymentMethod})</span>
                                        </td>
                    ...
                    {/* Resumo Financeiro */}
                    <div className="flex flex-wrap gap-2 mb-4 text-[10px] uppercase font-bold text-slate-500">
                        <span className="bg-slate-100 px-2 py-1 rounded border border-slate-200">Pix: R$ {report.totalPix.toFixed(2)}</span>
                        <span className="bg-slate-100 px-2 py-1 rounded border border-slate-200">Din: R$ {report.totalCash.toFixed(2)}</span>
                        <span className="bg-slate-100 px-2 py-1 rounded border border-slate-200">Card: R$ {(report.totalDebit + report.totalCredit).toFixed(2)}</span>
                    </div>

                    {report.notes && (
                        <div className="bg-yellow-50 p-3 rounded border border-yellow-200 text-xs text-yellow-800 mb-4 font-medium">
                            <span className="font-bold">Observações:</span> {report.notes}
                        </div>
                    )}
                    
                    {!isVal ? (
                        <button onClick={onAction} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-bold hover:bg-emerald-700 shadow-md flex justify-center items-center gap-2">
                            Confirmar Recebimento ({checkedCount}/{totalItems})
                        </button>
                    ) : (
                ...
                <div className="flex gap-4 items-center">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-xs text-slate-500">
                             ...
                             {!isVal && checkedCount > 0 && <span className="bg-indigo-50 text-indigo-600 px-2 rounded-full font-bold">{checkedCount}/{totalItems} ok</span>}
                        </div>
                    </div>
                </div>
                <div className="flex items-center gap-4">
                    <div className="text-right"><p className="text-xs text-slate-400">Total</p><p className={`text-lg font-bold ${isVal ? 'text-emerald-700' : 'text-indigo-700'}`}>R$ {order.grandTotal.toFixed(2)}</p></div>
                    {isExpanded ? <ChevronUp size={20} className="text-slate-300"/> : <ChevronDown size={20} className="text-slate-300"/>}
                </div>
            </div>
            {isExpanded && (
                <div className="border-t border-slate-100 bg-white p-4 animate-fade-in">
                     <div className="space-y-2 mb-4">
                        {order.items.map((item: any, idx: number) => (
                            <div 
                                key={idx} 
                                onClick={() => !isVal && onToggleItem && onToggleItem(order.id, idx)}
                                className={`bg-slate-50 p-3 rounded-lg border border-slate-100 flex flex-col sm:flex-row justify-between gap-2 text-xs transition-colors ${!isVal ? 'cursor-pointer hover:border-indigo-200' : ''} ${item.checked ? 'border-emerald-400 bg-emerald-50' : ''}`}
                            >
                                <div className="flex items-center gap-3">
                                    {!isVal && (
                                        <div className="text-slate-400">
                                            {item.checked ? <CheckCircle size={18} className="text-emerald-500"/> : <div className="w-4 h-4 border-2 border-slate-300 rounded-full"></div>}
                                        </div>
                                    )}
                                    <div>
                                        <div className="font-bold text-slate-700 flex items-center gap-2"><User size={12}/> {item.customerName}</div>
                                        <div className="text-slate-500 mt-1 flex gap-2"><span className="flex items-center gap-1"><Phone size={10}/> {item.customerPhone}</span><span className="bg-white px-1.5 rounded border border-slate-200 text-slate-600">{item.paymentMethod}</span></div>
                                    </div>
                                </div>
                                <div className="text-right"><p className="font-medium text-slate-800">{item.quantity}x {item.productName}</p><p className="text-slate-400">R$ {item.total.toFixed(2)}</p></div>
                            </div>
                        ))}
                     </div>
                    {!isVal ? (
                        <button onClick={onAction} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700 flex justify-center items-center gap-2 shadow-md">
                            <CheckCircle size={18}/> Validar Recebimento ({checkedCount}/{totalItems})
                        </button>
                    ) : (
                        <div className="flex gap-2">
