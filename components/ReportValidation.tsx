...
import { CheckCircle, ShoppingBag, Phone, User, Users, FileText, Filter, X, ChevronDown, ChevronUp } from 'lucide-react';
...
export const ReportValidation: React.FC<ReportValidationProps> = ({ reports, orders, onValidateReport, onValidateOrder }) => {
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      {/* --- HISTÓRICO VALIDADO (AGORA COM DETALHES) --- */}
      {(validatedReports.length > 0 || validatedOrders.length > 0) && (
        <div className="mt-12 pt-8 border-t border-slate-200">
            <h3 className="text-sm font-bold text-emerald-600 uppercase tracking-wider mb-6 flex items-center gap-2">
                <CheckCircle size={16} /> Histórico Validado (Clique para ver detalhes)
            </h3>
            
            <div className="space-y-4 opacity-75 hover:opacity-100 transition-opacity">
                {validatedReports.map(report => (
                    // Reutilizamos o ReportCard, mas ele saberá que está validado pelo status
                    <ReportCard 
                        key={report.id} 
                        report={report} 
                        isExpanded={expandedId === report.id} 
                        onToggle={() => toggleExpand(report.id)} 
                        onAction={() => {}} // Função vazia pois já está validado
                    />
                ))}
            ...
            {isExpanded && (
                <div className="border-t border-slate-100 bg-white p-4 animate-fade-in">
                    ...
                    {!isVal ? (
                        ...
                        <div className="w-full bg-emerald-50 text-emerald-600 py-2 rounded-xl font-bold text-center text-xs flex justify-center items-center gap-2 border border-emerald-100">
                            <CheckCircle size={14}/> Relatório Finalizado e Validado
                        </div>
                    )}
                </div>
            )}
