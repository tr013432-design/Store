                ...
                <div className="border-t border-slate-100 bg-white p-4 animate-fade-in">
                     <div className="bg-slate-50 rounded-lg border border-slate-200 overflow-hidden mb-4">
                        <table className="w-full text-xs text-left">
                            <thead className="bg-slate-100 font-semibold text-slate-500"><tr><th className="p-2">Qtd</th><th className="p-2">Item</th><th className="p-2 text-right">Valor</th></tr></thead>
                            <tbody className="divide-y divide-slate-100">
                                {report.items.map((i: any, idx: number) => (
                                    <tr key={idx}><td className="p-2">{i.quantity}</td><td className="p-2">{i.productName} <span className="text-[10px] text-slate-400">({i.paymentMethod})</span></td><td className="p-2 text-right">R$ {i.total.toFixed(2)}</td></tr>
                                ))}
                            </tbody>
                        </table>
                    </div>
            ...
            <div onClick={onToggle} className={`p-5 flex justify-between items-center cursor-pointer ${isVal ? 'bg-slate-50' : 'hover:bg-slate-50'}`}>
                <div className="flex gap-4 items-center">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-xs text-slate-500"><span>Resp: {order.volunteerName}</span>{isVal && <span className="bg-emerald-100 text-emerald-700 px-2 rounded-full font-bold">Aprovado</span>}</div>
                    </div>
                </div>
                <div className="flex items-center gap-4">
                     ...
                     <div className="space-y-2 mb-4">
                        {order.items.map((item: any, idx: number) => (
                            <div key={idx} className="bg-slate-50 p-3 rounded-lg border border-slate-100 flex flex-col sm:flex-row justify-between gap-2 text-xs">
                                <div><div className="font-bold text-slate-700 flex items-center gap-2"><User size={12}/> {item.customerName}</div><div className="text-slate-500 mt-1 flex gap-2"><span className="flex items-center gap-1"><Phone size={10}/> {item.customerPhone}</span><span className="bg-white px-1.5 rounded border border-slate-200 text-slate-600">{item.paymentMethod}</span></div></div>
