...
export const ReportValidation: React.FC<ReportValidationProps> = ({ 
  ...
  const handleAuthSubmit = () => {
    // Verifica se a senha bate com ALGUM admin
    const isAdmin = admins.some(admin => admin.password === authPassword);
    
    if (isAdmin && pendingAction) {
        if (pendingAction.type === 'REPORT') onValidateReport(pendingAction.id);
        if (pendingAction.type === 'ORDER') onValidateOrder(pendingAction.id);
        closeAuth();
    } else {
        alert("Senha incorreta! Apenas funcionários autorizados podem validar.");
        setAuthPassword('');
    }
  };
  ...
  // ... (Filtros - Igual ao código anterior)
      ...
      {authOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl animate-fade-in">
                ...
                <button onClick={handleAuthSubmit} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700 transition-colors">
                    Confirmar Validação
                </button>
            </div>
        </div>
      ...
      <div>
        ...
        {pendingReports.length === 0 ? <p className="text-sm text-slate-400 italic">Nenhum relatório pendente.</p> : 
            pendingReports.map(report => (
                <ReportCard key={report.id} report={report} isExpanded={expandedId === report.id} onToggle={() => toggleExpand(report.id)} 
                    onAction={() => requestValidation('REPORT', report.id)} // <--- CHAMA O MODAL
                    onToggleItem={onToggleReportItem} 
                />
            ))
        }
      </div>
      ...
      {pendingOrders.length > 0 && (
        <div className="mt-6">
            ...
            {pendingOrders.map(order => (
                <OrderCard key={order.id} order={order} isExpanded={expandedId === order.id} onToggle={() => toggleExpand(order.id)} 
                    onAction={() => requestValidation('ORDER', order.id)} // <--- CHAMA O MODAL
                    onToggleItem={onToggleOrderItem}
                />
            ))}
        </div>
      )}
...
// ... (ReportCard e OrderCard permanecem idênticos ao anterior)
        ...
        <div className={`mb-3 bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${isExpanded ? (isVal ? 'shadow-md border-emerald-200' : 'shadow-lg border-amber-400 ring-1 ring-amber-400') : 'shadow-sm border-slate-200'}`}>
            <div onClick={onToggle} className={`p-5 flex justify-between items-center cursor-pointer ${isVal ? 'bg-slate-50' : 'hover:bg-slate-50'}`}>
                <div className="flex gap-4 items-center">
                    ...
                    <div><h4 className="font-bold text-slate-800">{report.serviceType}</h4><div className="flex items-center gap-2 text-xs text-slate-500"><span>Resp: {report.volunteerName}</span>{isVal && <span className="bg-emerald-100 text-emerald-700 px-2 rounded-full font-bold">Validado</span>}{!isVal && checkedCount > 0 && <span className="bg-amber-50 text-amber-600 px-2 rounded-full font-bold">{checkedCount}/{totalItems} conferidos</span>}</div></div>
                </div>
