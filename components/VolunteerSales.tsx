...
import { Product, PaymentMethod, Category, DailyReport, ReportItem, CartItem } from '../types';
import { Plus, Minus, Trash2, User, Calendar, Clock, Church, FileText, Search, CreditCard, Save, Barcode } from 'lucide-react';
...
export const VolunteerSales: React.FC<VolunteerSalesProps> = ({ products, onSubmitReport }) => {
  // Cabeçalho
  ...
  // Lista de Itens do Relatório (Temporária)
  const [reportItems, setReportItems] = useState<ReportItem[]>([]);
  
  // Notas (Retirada do Devocional)
  const [notes, setNotes] = useState('');

  // Estados de "Venda Rápida" (para adicionar na lista)
  ...
  const handleSearchKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
        ...
        if (foundProduct) {
            // Seleciona o item para escolher quantidade e pagto
  ...
  const addLineToReport = (method: PaymentMethod) => {
    ...
    setCurrentItem(null); // Limpa seleção
    
    // Volta foco pro leitor
    setTimeout(() => searchInputRef.current?.focus(), 100);
  };

  // 3. Finaliza Relatório Completo
  ...
  return (
    <div className="flex flex-col h-full gap-6 pb-24 lg:pb-0">
      ...
      {/* 1. Cabeçalho (Fixo igual papel) */}
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 grid grid-cols-1 md:grid-cols-4 gap-4">
        <div><label className="text-xs font-bold text-slate-400">VOLUNTÁRIO</label><input value={volunteerName} onChange={e => setVolunteerName(e.target.value)} className="w-full font-medium text-slate-700 border-b border-slate-200 outline-none focus:border-indigo-500" placeholder="Seu nome" /></div>
        <div>
            <label className="text-xs font-bold text-slate-400">CULTO / EVENTO</label>
            <input list="services" value={serviceType} onChange={e => setServiceType(e.target.value)} className="w-full font-medium text-slate-700 border-b border-slate-200 outline-none focus:border-indigo-500" placeholder="Selecione..." />
            <datalist id="services">{defaultServices.map(s => <option key={s} value={s} />)}</datalist>
        </div>
        <div><label className="text-xs font-bold text-slate-400">DATA</label><input type="date" value={reportDate} onChange={e => setReportDate(e.target.value)} className="w-full font-medium text-slate-700 border-b border-slate-200 outline-none" /></div>
        <div><label className="text-xs font-bold text-slate-400">HORA</label><input type="time" value={reportTime} onChange={e => setReportTime(e.target.value)} className="w-full font-medium text-slate-700 border-b border-slate-200 outline-none" /></div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
        
        {/* ESQUERDA: Adicionar Item (Leitor) */}
        <div className="lg:col-span-1 flex flex-col gap-4">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                {/* Se produto selecionado, mostra controles */}
                {currentItem ? (
            ...
            <div className="bg-yellow-50 p-4 rounded-2xl border border-yellow-100 flex-1">
                ...
                <textarea 
                    ...
                    className="w-full h-full bg-transparent resize-none outline-none text-sm text-yellow-900 placeholder-yellow-900/40 min-h-[100px]"
                    placeholder="Ex: Cláudia Oliveira - Chama (Pago depois)..."
                />
            </div>
        </div>

        {/* DIREITA: O Relatório sendo construído (Tabela) */}
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg border border-slate-200 flex flex-col overflow-hidden">
                        ...
                        {reportItems.map((item, idx) => (
                            <tr key={idx} className="hover:bg-indigo-50/50">
                                ...
                                <td className="p-3 text-right">
                                    <button onClick={() => setReportItems(prev => prev.filter((_, i) => i !== idx))} className="text-slate-300 hover:text-red-500"><Trash2 size={16}/></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50">
                ...
                <button 
                    ...
                    className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 flex justify-center items-center gap-2 transition-all"
                >
